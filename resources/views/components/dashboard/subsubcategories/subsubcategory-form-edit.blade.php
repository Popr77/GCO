...
<div class="container mt-5">
    <div class="row justify-content-center">
        <div class="col-6">
            <form method="POST" action="{{ url('dashboard/subsubcategories/' . $subsubcategory->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <select class="form-select form-select-lg mb-3 form-control" aria-label=".form-select-lg example" name="subcategory_id">
                        @foreach($subcategories as $subcategory)
                            <option value="{{$subcategory->id}}" {{$subsubcategory->subcategory->id == $subcategory->id ? 'selected' : ''}}>{{$subcategory->name}}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <form method="POST" action="{{ url('dashboard/subsubcategories/' . $subsubcategory->id) }}" id="delete_form">
                @csrf
                @method('DELETE')
            </form>
        </div>
